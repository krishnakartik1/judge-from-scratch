...
def upload_model_remote(target: str, card_text: str, dry_run: bool) -> dict:
    ...
    api.upload_folder(
        ...
        commit_message=f"Stage 9 publish: {target}",
    )
...
def upload_gguf_remote(
    ...
    # Hard-link (no copy) the GGUFs into the flat staging dir under
    # their published filenames; HF upload_folder reads from staging.
    ...
    api.upload_folder(
        ...
        repo_id=info["repo_id"],
        repo_type=info["repo_type"],
        commit_message="Stage 9 publish: gguf",
    )
    return {
        "target": "gguf",
        "repo_id": info["repo_id"],
